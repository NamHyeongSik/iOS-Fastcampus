...
class RepositoryListViewController : UITableViewController {
    private let userName = "NamHyeongSik"
    ...
    @objc func refresh() {
        DispatchQueue.global(qos: .background).async { [weak self] in
            ...
            self.fetchRepositories(of: self.userName)
        }
    }
    
    func fetchRepositories(of userName: String) {
        Observable.from([userName])
            ...
            .filter { responses, _ in
                return 200..<300 ~= responses.statusCode
            }
